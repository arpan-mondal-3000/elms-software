...
import { type LeaveDetail, type LeaveSummary } from "../../lib/types";
          ...
          {SUMMARY.map((s) => (
            <Card key={s.label} className="p-5">
              <CardContent className="p-0">
                <p className="text-sm text-text-secondary-light dark:text-text-secondary-dark">{s.label}</p>
                <div className="mt-2 text-2xl font-bold">{s.daysLeft} <span className="text-sm font-medium text-text-secondary-light dark:text-text-secondary-dark">Days Left</span></div>
              </CardContent>
            </Card>
          ))}
        </div>
