...
export const getAllAdminLeaves = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Failed to fetch admin leave requests!",
    });
  }
};
  ...
  try {
    ...
    if (startOfLeave <= currentDate) {
      ...
      return res.status(406).json({
        success: false,
        message: "Leave request cannot be created for past!",
      });
    }
    let totalDays = 0;
    ...
    if (startDate <= endDate) {
      totalDays =
        Math.round(Math.abs((startOfLeave - endOfLeave) / oneDayInMs)) + 1;
    } else {
      console.error(
        "Start date must be less than or equal to end date of leave!"
      );
      return res.status(406).json({
        success: false,
        message: "Start date must be less than or equal to end date of leave!",
      });
    }
    ...
    if (!leaveBalance) {
      ...
      const [leaveTypeData] = await db
        .select()
        .from(leaveTypes)
        .where(eq(leaveTypes.id, leaveType))
        .limit(1);
      ...
      if (totalDays > maxDaysPerYear) {
        console.err(
          "Total number of days exceeded balance for user ",
          employeeId
        );
        return res.status(406).json({
          success: false,
          message: "Total number of days exceeded your balance!",
        });
      }
...
export const updateLeaveRequest = async (req, res) => {
  try {
    ...
    // Fetch leave request
    const [oldLeaveRequest] = await db
      .select()
      .from(leaveRequests)
    ...
    if (oldLeaveStart <= currentDate) {
      ...
      return res
        .status(406)
        .json({ success: false, message: "Leave period has already started!" });
    }
    ...
    if (Number(leaveType) === Number(oldLeaveRequest.leaveType)) {
      [leaveBalance] = await db
        .select()
        .from(leaveBalances)
        .where(
          and(
            eq(leaveBalances.employeeId, employeeId),
            eq(leaveBalances.leaveType, oldLeaveRequest.leaveType)
          )
        )
        .limit(1);
    } else {
    ...
    if (oldLeaveRequest.status === "approved") {
      await db
        ...
        .set({
          usedDays:
            Number(leaveBalance.usedDays) - Number(oldLeaveRequest.totalDays),
          remainingDays:
            Number(leaveBalance.remainingDays) +
            Number(oldLeaveRequest.totalDays),
          updatedAt: new Date(),
        })
        .where(
          and(
...
export const cancelLeaveRequest = async (req, res) => {
  try {
    ...
    if (oldLeaveRequest.status === "approved") {
      ...
      const [oldLeaveBalance] = await db
        .select()
        .from(leaveBalances)
        .where(
          and(
      ...
      await db
        ...
        .set({
          ...
          remainingDays:
            oldLeaveBalance.remainingDays + oldLeaveRequest.totalDays,
          updatedAt: new Date(),
        })
        .where(
          and(
    ...
    const [cancelledLeaveRequest] = await db
      .update(leaveRequests)
      .set({
        status: "canceled",
        updatedAt: new Date(),
    ...
    return res
      .status(500)
      .json({ success: false, message: "Failed to cancel leave request!" });
  }
};
  ...
  try {
    ...
    if (status === "approved") {
      ...
      await db
        ...
        .set({
          usedDays:
            Number(leaveBalance.usedDays) + Number(leaveRequest.totalDays),
          remainingDays:
            Number(leaveBalance.remainingDays) - Number(leaveRequest.totalDays),
          updatedAt: new Date(),
        })
        .where(
          and(
      ...
      return res.status(406).json({
        success: false,
        message: "Leave status should either be approved or rejected!",
      });
    }
  } catch (err) {
    ...
    return res.status(500).json({
      success: false,
      message: "Failed to set status of leave request!",
    });
  }
};

export const getEmployeeLeaveOverview = async (req, res) => {
  try {
    const { id: employeeId } = req.user;
    ...
    const rows = await db
      .select({
        ...
        title: leaveTypes.name,
        total: leaveTypes.maxDaysPerYear,
        used: leaveBalances.usedDays,
        remaining: leaveBalances.remainingDays,
      })
      .from(leaveBalances)
      .innerJoin(leaveTypes, eq(leaveBalances.leaveType, leaveTypes.id))
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch leave overview" });
  }
};
