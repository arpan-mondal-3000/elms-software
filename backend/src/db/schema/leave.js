import { integer, pgTable, varchar, pgEnum, text, date } from "drizzle-orm/pg-core";
import { timestamps } from "./columns.helper.js";
import { employees, admins } from "./users.js";
import { relations } from "drizzle-orm";

// Enums
export const statuses = pgEnum("statuses", ["pending", "approved", "rejected", "canceled"]);
...
export const leaveBalances = pgTable("leave_balances", {
    id: integer("id").primaryKey().generatedAlwaysAsIdentity(),
    employeeId: integer("employee_id").notNull().references(() => employees.employeeId),
    leaveType: integer("leave_type").notNull().references(() => leaveTypes.id),
...
export const leaveRequests = pgTable("leave_requests", {
    id: integer("id").primaryKey().generatedAlwaysAsIdentity(),
    employeeId: integer("employee_id").notNull().references(() => employees.employeeId),
    approverId: integer("approver_id").notNull().references(() => admins.adminId),
    leaveType: integer("leave_type").notNull().references(() => leaveTypes.id),
